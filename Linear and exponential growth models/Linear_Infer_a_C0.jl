#=
    ...
    Produces inference and information geometry results for the linear model, estimating θ = (a,C(0))
=#
...
function LinearSol(θ,t)
    Linear = LinearSolAllParams(θ,t)
    return Linear
end

#Loglikleihood function
llfun = (θ) -> sum([ loglikelihood(Normal(LinearSol(θ[1:2],t),σ),[Ndata[i]]) for (i,t) in enumerate(Ntime)])
#Compute MLE and MLL
...
scatter!(pltsol,Ntime, Ndata,label="data",xlabel = "time",ylabel = "Population density (%)",title = "Linear")
#Add solution based on true parameters to the plot
Linear_sol=zeros(length(Tspan))
for (i,t) in enumerate(Tspan)
Linear_sol[i] = LinearSol(θtrue,t)
end
plot!(pltsol,(Tspan,Linear_sol),label = "Underlying model",color = :red, markerstrokewidth = 1.5,legend=:bottomright,ylims=(0,1.5))

#Specify range for loglikelihood and scalar curvature surface plots
arange = range(0.25,1.75,length=100)
C0range = range(0.45,0.9,length=100)
...
#Construct Fisher information matrix for the linear model, estimating θ = (a,C(0))
function Fisher(θ)
