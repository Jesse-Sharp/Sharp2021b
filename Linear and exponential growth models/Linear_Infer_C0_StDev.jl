#=
    Linear_Infer_C0_StDev.jl
    Produces inference and information geometry results for the linear model, estimating θ = (C0,σ)
=#
...
θ0 = [C0_0,σ0]
C0_min = 0.0 #lower bound on C0
C0_max = 1.0 #upper bound on C0
...
function LinearSol(θ,t)
    (a,C0) = θ
    Linear = LinearSolAllParams([a;C0],t)
    return Linear
end

#Loglikleihood function
llfun = (θ) -> sum([ loglikelihood(Normal(LinearSol([a;θ[1]],t),θ[2]),[Ndata[i]]) for (i,t) in enumerate(Ntime)])
#Compute MLE and MLL
...
#Construct Fisher information matrix for the linear model, estimating θ = (C(0),σ)
function Fisher(θ)
