#=
    ...
    Produces inference and information geometry results for the exponential model, estimating θ = (a,σ)
=#
...
function ExponentialSol(θ,t)
    Exponential = ExponentialSolAllParams([θ[1];C0],t)
    return Exponential
end

#Loglikleihood function
llfun = (θ) -> sum([ loglikelihood(Normal(ExponentialSol([θ[1];C0],t),θ[2]),[Ndata[i]]) for (i,t) in enumerate(Ntime)])
#Compute MLE and MLL
...
#Construct Fisher information matrix for the exponential model, estimating θ = (a,σ)
function Fisher(θ)
