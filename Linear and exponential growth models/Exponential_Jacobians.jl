#=
    Exponential_Jacobians.jl
    Forms the Jacobian of the exponential model with respect to the parameters;  as required for computing the Fisher information matrix
=#

#With unknown parameters θ = (a,C(0))
function Exponential_Jacobian_aC0(θ,T)
...
function Exponential_Jacobian_a_StDev(θ,T)
    J = zeros(2*length(T),2)
    for (i,t) in enumerate(T)
...
function Exponential_Jacobian_C0_StDev(θ,T)
    J = zeros(2*length(T),2)
    for (i,t) in enumerate(T)
    #Every second row is d/dσ (mean,σ)
