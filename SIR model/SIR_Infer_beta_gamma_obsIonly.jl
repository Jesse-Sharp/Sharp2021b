#=
    SIR_Infer_Params_beta_gamma_obsIonly.jl
    Produces inference and information geometry results for the SIR model where only I is observed, estimating θ = (β,γ)
=#
...
llfun = θ -> sum([ loglikelihood(Normal(SIRSol(θ[1:2],t,[S₀;I₀;R₀])(t)[2],σ),[Ndata[i]]) for (i,t) in enumerate(Ntime)])
#Compute MLE and MLL
...
scatter!(pltsol,Ntime, Ndata,label="Data",xlabel = "Time (days)",ylabel = "Population proportion",title = "SIR infer beta gamma",markercolor=[:red],markersize = 5) #add synthetic data to plot
...
#Construct Fisher information matrix for the SIR model where only I is observed, estimating θ = (β,γ)
function Fisher(θ)
    FIM = Fisher_βγ_obsIonly([θ;S₀;I₀;R₀],σ,TimePoints,NperT)
    return FIM
end
...
ScalarSurfaceplot = heatmap(γrange,βrange,ScalarSurface,xlabel = "γ", ylabel = "β",right_margin = 20Plots.mm,xtickfontsize=16,ytickfontsize=16,xguidefontsize=16,yguidefontsize=16,legendfontsize=16,color=:lajolla,clims = (0,0.005))
