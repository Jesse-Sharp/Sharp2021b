#=
    SIR_Infer_Params_beta_StDev.jl
    Produces inference and information geometry results for the SIR model where S, I anr R are observed, estimating θ = (β,σ)
=#
...
llfun = θ -> sum([ loglikelihood(MvNormal(SIRSol([θ[1];γ],t,[S₀;I₀;R₀])(t),θ[2]^2*I),[Ndata[i,:]]) for (i,t) in enumerate(Ntime)])
#Compute MLE and MLL
...
σrange = range(0.02,0.05,length=100)
...
#Construct Fisher information matrix for the SIR model where S, I and R are observed, estimating θ = (β,σ)
function Fisher(θ)
    FIM = Fisher_βσ([θ[1];γ;S₀;I₀;R₀],θ[2],TimePoints,NperT,3)
    return FIM
end
